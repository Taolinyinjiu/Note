# Jetson Orin NX: 当前 Sunray_V2_PR 仓库内使用 OpenCV 4.6 CUDA 编译 VINS-Fusion-GPU
...
```bash
localization/third_party_localization/vins-fusion-gpu/third_party/vision_opencv
```
...
test -f localization/third_party_localization/vins-fusion-gpu/third_party/vision_opencv/cv_bridge/package.xml
test -f localization/third_party_localization/vins-fusion-gpu/third_party/vision_opencv/VENDOR_VERSION.md
```

这个操作不会移动 VINS-Fusion-GPU。VINS 仍然在：

```bash
localization/third_party_localization/vins-fusion-gpu/vins_estimator
localization/third_party_localization/vins-fusion-gpu/camera_models
localization/third_party_localization/vins-fusion-gpu/loop_fusion
localization/third_party_localization/vins-fusion-gpu/global_fusion
```
...
catkin_make \
  --source localization/third_party_localization/vins-fusion-gpu \
...
```bash
source space: localization/third_party_localization/vins-fusion-gpu
...
source /opt/ros/noetic/setup.bash
source /opt/opencv-4.6.0-cuda/setup.sh
source /home/taolin/Documents/GitHub/Sunray_V2_PR/devel/setup.bash
```
...
期望 `vins` 和 `cv_bridge` 都来自当前 `Sunray_V2_PR/devel` overlay，而不是只有 `/opt/ros/noetic`。
...
这适合完全隔离的部署方式，但不满足“VINS 仍在当前 Sunray_V2_PR 仓库、编译输出仍在当前仓库”的要求。
...
### 为什么要在 localization/third_party_localization/vins-fusion-gpu 下放 vision_opencv？

因为 `catkin_make --source localization/third_party_localization/vins-fusion-gpu` 只会扫描这个 source space 下的包。
...
/opt/opencv-4.6.0-cuda
        ↓
使用仓库内已固化的 localization/third_party_localization/vins-fusion-gpu/third_party/vision_opencv
        ↓
catkin_make --source localization/third_party_localization/vins-fusion-gpu --build build/vins-fusion-gpu
        ↓
devel/lib/libcv_bridge.so
devel/lib/vins/vins_node
devel/lib/libvins_lib.so
        ↓
当前 Sunray_V2_PR 仓库内运行 VINS CUDA 版本
```
